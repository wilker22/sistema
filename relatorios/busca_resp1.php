...
if(isset($_POST['pn'])){
	...
	$dataString = '';
		      ...
		      $stmt = $mysqli->prepare("SELECT  `id_pedido` FROM responsaveis WHERE  `id_user` = ? AND  `id_area` = ? ");
              $stmt->bind_param('ss' $id_user, $id_area);
              $stmt->execute();
              $stmt->store_result();
              ...
              $sql->close();
				...
				$dataString .= $nome_representante.'|'.$ref_trabalho_pedido.'|'.$id_pedido.'|'.$id_ordem.'|'.$databr.'|'.$nome_cliente.'|'.$cnpj_cliente.'||';
...
ORDER BY id_pedido DESC LIMIT ?, ?");
    ...
    $stmt->bind_param('sss', $id_user, $limit, $rpp);
    $stmt->execute();
    $stmt->store_result();
    if ($stmt->error ) printf("Error: %s.\n", $stmt->error);	
$stmt->bind_result($id_pedido);
    ...
    $sql = $mysqli->prepare("SELECT `id_pedido` , `data_pedido` , `nome_cliente` , `cnpj_cliente`, `ref_trabalho_pedido`, `id_representante`
FROM `pedido_megalabel`
WHERE `id_pedido` = ?
NATURAL JOIN cliente_megalabel
ORDER BY id_pedido");
    $sql->bind_param('sss', $id_pedido);
    $sql->execute();
    $sql->store_result();
