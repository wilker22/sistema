...
if(isset($_POST['devolv_pcp'])){
	
	
	
$stmt = $mysqli->prepare("UPDATE fluxo
SET `id_area`=1, `id_user`=5
WHERE id_pedido = ? ");
...
$stmt=$mysqli->prepare("INSERT INTO `ordem_producao_megalabel`
(`id_pedido` , `id_ordem` , `data_aprov_ordem`, `largura_papel_ordem` ,	`metros_lineares_ordem` ,	`n_engrenagem_ordem` ,	`faca_ordem` ,	`rotulo_ordem`, `data_entrega_prevista`, `qtd_final`)
VALUES ( ? , NULL , ?, ?, ?, ?, ?, ?, ?, 0)");
$stmt->bind_param('ssssssss', $id_pedido, $data, $largura_papel_ordem, $metros_lineares_ordem, $n_engrenagem_ordem, $faca_ordem, $rotulo_ordem, $final);
$stmt->execute();
$stmt->store_result();
	        if ($stmt->error ) printf("Error: %s.\n", $stmt->error);	 
$stmt->close();	           	           
...
$message = "Caros,
O pedido realizado no dia $data_pedido2 foi aprovado e teve sua ordem gerada.
O mesmo tem previsão de entrega para o dia $final2";
...
mail($to,$subject,$message,$headers);
    	...
    	if($nome_tinta_imp_pedido0 == ""){ $t_0=""; }
	else{ $t_0 = ","; }
    	
    	if($nome_tinta_imp_pedido1 == ""){ $t_1=""; }
	else{ $t_1 = ","; }
	
    	if($nome_tinta_imp_pedido2 == ""){ $t_2=""; }
	else{ $t_2 = ","; }
	
	
    $tinta_impressao_final = $nome_tinta_imp_pedido0.$t_0.$nome_tinta_imp_pedido1.$t_1.$nome_tinta_imp_pedido2.$t_2;
...
Tinta Impressão: $tinta_impressao_final
...
<link rel="stylesheet" href="/megalabel/style.css">
	...
	document.getElementById('boxverpedido').style.height='1173px';
	document.getElementById('aprov_gera_ordem').style.display='none'; 		    
    }
	...
	document.getElementById('aprov_gera_ordem').style.display='inline';    
    }
...
<form action="arteverpedido.php"  id="bt_arte_devolv" method="post"> 
<input type="hidden" name="id_pedido" value="$id_pedido"/>
<input  id="devolv_pcp" type="submit" name="devolv_pcp" value="Devolver para PCP"/>
</form>
_END;
...
if ($rows==0){
 ...
 $data_aprov_ordem_br= implode("/",array_reverse(explode("/",$data)));
echo <<< _END
<input  id="aprov_gera_ordem" type="submit" name="aprov_gera_ordem" value="Aprovar/ Gerar Ordem" onclick="Verboxordem()"/>
</li>
</li>
_END;
}
...
<input type="hidden" name="id_pedido" value="$id_pedido"/>
<input  id="ver_ordem" type="submit" name="ver_ordem" value="Visualizar Ordem"/>
</form>
</li>
</li>
    </ul>
...
<input type="submit" name="aprov_gera_ordem" value="Aprovar/ Gerar Ordem"/>
