...
<center>
  ...
  <table align="center" width="986px" height="62px" border="0" cellpadding="0" cellspacing="0">
	...
	<div class="boxfila"><div class="headerfila">Fila de pedidos Arte usuário: $nome_user</div>
	<br /> 	
...
WHERE  `id_area` ='3' AND `id_user`= ? ORDER BY id_pedido DESC");
	...
	/*
	$sql = $mysqli->prepare("SELECT `id_pedido`, `data_pedido`, `nome_cliente`, `cnpj_cliente` FROM `pedido_megalabel` NATURAL JOIN cliente_megalabel NATURAL JOIN fluxo
WHERE  `id_area` ='3' AND `id_user`= ? ORDER BY id_pedido DESC");
	$sql->bind_param('s', $id_user);
	$sql->execute();
	$sql->bind_result($id_pedido, $data_pedido, $nome_cliente, $cnpj_cliente);
	$sql->store_result();
