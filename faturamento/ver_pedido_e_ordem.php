...
WHERE id_pedido= ? ");
	...
	$stmt= $mysqli->prepare("INSERT INTO `data_nf`
VALUES (?, ?)");
    $stmt->bind_param('ss', $id_pedido, $data_final);	        
	$stmt->execute();
	$stmt->store_result();
	            if ($stmt->error ) printf("Error: %s.\n", $stmt->error);	
    $stmt->close();	
...
echo '<META HTTP-EQUIV=Refresh CONTENT="0; URL='.$url.'">';
 ...
 if (isset($_POST['ver_pedido_e_ordem'])){
...
<div id="header_ver_ordem">Ordem de Serviço: $id_ordem </div>
...
<form action="ver_pedido_e_ordem.php"  id="bt_arte_devolv" method="post"> 
<input type="hidden" name="id_pedido" value="$id_pedido"/>
<input  id="devolv_revi" type="submit" name="devolv_revi" value="Devolver para Revisão"/>
</form>


<form action="ver_pedido_e_ordem.php"  id="bt_arte_devolv" method="post"> 
<input type="hidden" name="id_pedido" value="$id_pedido"/>
<input  id="finalizar" type="submit" name="finalizar" value="Finalizar Pedido"/>
</form>
</li>
</div>
</ul>
