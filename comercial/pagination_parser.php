...
if(isset($_POST['pn'])){
	...
	$stmt = $mysqli->prepare("SELECT `id_pedido` , `data_pedido` , `nome_cliente` , `cnpj_cliente`, `ref_trabalho_pedido`
...
$stmt->bind_result($id_pedido, $data_pedido, $nome_cliente, $cnpj_cliente, $ref_trabalho_pedido);
...
while($stmt->fetch()){
	      ...
	      $databr = implode("-",array_reverse(explode("-",$data_pedido)));
	  	    ...
	  	    $sql=$mysqli->prepare("SELECT `id_area`, `id_pedido` FROM `fluxo` WHERE `id_pedido` = ?");
	      $sql->bind_param('s', $id_pedido);
	      $sql->execute();
	      $sql->store_result();
    if ($sql->error ) printf("Error: %s.\n", $sql->error);	
	      $sql->bind_result($id_area, $id_pedido1);
	      $sql->fetch();
	      ...
	      $sql->close();
              ...
              $sql = $mysqli->prepare("SELECT `id_ordem`, `data_entrega_prevista` FROM `ordem_producao_megalabel` WHERE `id_pedido` = ? ");
              $sql->bind_param('s', $id_pedido1);
              $sql->execute();
              $sql->store_result();
              $sql->bind_result($id_ordem, $data_entrega_prevista);
              if($sql->num_rows > 0){
		...
		$dataString .= $id_pedido1.'|'.$databr.'|'.$nome_cliente.'|'.$cnpj_cliente.'|'.$area_final.'|'.$ref_trabalho_pedido.'|'.$id_ordem_final.'|'.$nf_databr.'||';
